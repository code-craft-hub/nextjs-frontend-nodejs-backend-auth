...
export class AuthService {
  ...
  async register(data: RegisterRequest, ipAddress: string): Promise<{ user: User; tokens: AuthTokens }> {
    try {
  ...
  async login(data: LoginRequest, ipAddress: string, userAgent: string): Promise<{ user: User; tokens: AuthTokens }> {
    try {
  ...
  async refreshTokens(refreshToken: string, ipAddress: string): Promise<AuthTokens> {
    try {
      ...
      const { payload } = await jwtVerify(refreshToken, JWT_REFRESH_SECRET) as { payload: RefreshTokenPayload };
  ...
  private async generateTokens(
    ...
    const sessionKey = `session:${sessionId}`;
    await redisClient.setex(sessionKey, 7 * 24 * 60 * 60, JSON.stringify(sessionData)); // 7 days TTL
