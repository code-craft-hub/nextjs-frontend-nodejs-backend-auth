...
export const jobsApi = {
  ...
  autoApplyGet: (params?: any, token?: string) => {
    return api.get<PaginatedResponse<JobPost>>(
  ...
  getJob: async (id: string, token?: string) => {
    try {
      const data = await api.get<{ data: JobPost }>(
        `/job-posts/${id}`,
        { token },
      );
      return data;
  ...
  // Create job
  createJob: (data: any) => api.post<JobPost>("/job-recommendations", data),

  // Update job
  updateJob: (id: string, data: any) => api.patch<JobPost>(`/job-posts/${id}`, data),
  ...
  bulkUpdateJobs: (ids: string[], data: any) =>
    api.post<JobPost[]>("/job-recommendations/bulk/update", { ids, data }),

  bulkDeleteJobs: (ids: string[]) =>
    api.post<void>("/job-recommendations/bulk/delete", { ids }),

  bulkChangeStatus: (ids: string[], status: any) =>
    api.post<JobPost[]>("/job-recommendations/bulk/status", { ids, status }),

  // Duplicate job
  duplicateJob: (id: string) =>
    api.post<JobPost>(`/job-recommendations/${id}/duplicate`),
  ...
  searchJobs: (query: string, limit: number = 10) =>
    api.get<JobPost[]>("/job-recommendations/search", {
      params: { q: query, limit },
    }),

  // Get similar jobs (recommendation engine)
  getSimilarJobs: (id: string, limit: number = 5) =>
    api.get<JobPost[]>(`/job-recommendations/${id}/similar`, {
      params: { limit },
    }),
  ...
  getBookmarkedJobs: (
    ...
    api.post<PaginatedResponse<JobPost>>(
