...
class TextExtractionService {
  ...
  static validateFile(file: File): { valid: boolean; error?: string } {
    ...
    if (!allSupportedTypes.includes(file.type)) {
      return {
        valid: false,
        error: 'Unsupported file type. Please upload an image, PDF, or Word document.',
      };
    }
  ...
  private static async extractFromImage(file: File): Promise<string> {
    try {
      ...
      if (!data.text || data.text.trim().length === 0) {
        throw new Error('No text detected in image. The image may be blank or text quality is too low.');
      }
...
export const DocumentTextExtractor = () => {
  const { processDocument, isProcessing, error, currentFile, clearError, clearFile } =
