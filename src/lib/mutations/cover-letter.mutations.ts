...
export function useCreateCoverLetterMutation() {
  ...
  return useMutation({
    mutationFn: (data: CreateCoverLetterData) =>
      coverLetterApi.createCoverLetter(data),
    onMutate: async (newCoverLetter) => {
      await queryClient.cancelQueries({
        queryKey: queryKeys.coverLetters.lists(),
      });

      const previousCoverLetters = queryClient.getQueryData(
        queryKeys.coverLetters.lists(),
      );
...
export function useUpdateCoverLetterMutation() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      const previousCoverLetter = queryClient.getQueryData(
        queryKeys.coverLetters.detail(id),
      );

      queryClient.setQueryData<CoverLetter>(
        queryKeys.coverLetters.detail(id),
        (old) => {
          if (!old) return old;
        ...
        (old) => {
          ...
          return {
            ...
            data: old.data.map((cl) =>
              cl.id === id
                ? { ...cl, ...data, updatedAt: new Date().toISOString() }
                : cl,
            ),
          };
        },
    ...
    onError: (_err, { id }, context) => {
      if (context?.previousCoverLetter) {
        queryClient.setQueryData(
          queryKeys.coverLetters.detail(id),
          context.previousCoverLetter,
        );
      }
    },
    onSettled: (_err, _id, { id }) => {
      invalidateCoverLetterDetail(queryClient, id);
      invalidateCoverLetterLists(queryClient);
    },
  });
}
