...
export const jobsApi = {
  ...
  getJobs: (params?: any) =>
    api.get<PaginatedResponse<JobType>>("/job-recommendations/search/ilike", { params }),
  ...
  // Update job
  updateJob: (id: string, data: any) => api.patch<JobType>(`/job-recommendations/${id}`, data),
  ...
  // Duplicate job
  duplicateJob: (id: string) => api.post<JobType>(`/job-recommendations/${id}/duplicate`),
  ...
  searchJobs: (query: string, limit: number = 10) =>
    api.get<JobType[]>("/job-recommendations/search", { params: { q: query, limit } }),

  // Get similar jobs (recommendation engine)
  getSimilarJobs: (id: string, limit: number = 5) =>
    api.get<JobType[]>(`/job-recommendations/${id}/similar`, { params: { limit } }),
