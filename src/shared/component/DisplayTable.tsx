...
// ─── Default Data ──────────────────────────────────────────────────────────────
const DEFAULT_JOBS = [
  { id: "1", title: "Technical Support Specialist", employmentType: "Full Time", companyName: "Google",       location: "Lagos", salaryInfo: "$15K–$20K", postedAt: "2 days ago", matchScore: 100, link: "#", logoType: "google"      },
  { id: "2", title: "Technical Support Specialist", employmentType: "Full Time", companyName: "YouTube",      location: "Lagos", salaryInfo: "$15K–$20K", postedAt: "2 days ago", matchScore: 100, link: "#", logoType: "youtube"     },
  { id: "3", title: "Technical Support Specialist", employmentType: "Full Time", companyName: "Reddit",       location: "Lagos", salaryInfo: "$15K–$20K", postedAt: "2 days ago", matchScore: 100, link: "#", logoType: "reddit"      },
  { id: "4", title: "Technical Support Specialist", employmentType: "Full Time", companyName: "Product Hunt", location: "Lagos", salaryInfo: "$15K–$20K", postedAt: "2 days ago", matchScore: 100, link: "#", logoType: "producthunt" },
  { id: "5", title: "Technical Support Specialist", employmentType: "Full Time", companyName: "Instagram",    location: "Lagos", salaryInfo: "$15K–$20K", postedAt: "2 days ago", matchScore: 100, link: "#", logoType: "instagram"   },
  { id: "6", title: "Technical Support Specialist", employmentType: "Full Time", companyName: "Slack",        location: "Lagos", salaryInfo: "$15K–$20K", postedAt: "2 days ago", matchScore: 100, link: "#", logoType: "slack"       },
];

// ─── CompanyLogo ───────────────────────────────────────────────────────────────
function CompanyLogo({ logoType }) {
  const logo = LOGOS[logoType] ?? LOGOS.google;
  return (
    <div
      className={cn(
        "flex items-center justify-center shrink-0 w-[52px] h-[52px] rounded-xl overflow-hidden",
        logo.wrapperCn,
      )}
    >
      {logo.svg}
...
// ─── JobCard ───────────────────────────────────────────────────────────────────
export function JobCard({ job, isSelected, onSelect }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        {/* Logo */}
        <CompanyLogo logoType={job.logoType} />
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center flex-wrap gap-x-3 gap-y-1">
            ...
            <span className="inline-flex items-center gap-1 text-gray-500 text-[12px] sm:text-[13px]">
              <DollarSign size={12} strokeWidth={2} className="text-gray-400 shrink-0" />
              {job.salaryInfo}
            </span>
            <span className="inline-flex items-center gap-1 text-gray-500 text-[12px] sm:text-[13px]">
              <CalendarDays size={12} strokeWidth={2} className="text-gray-400 shrink-0" />
              {job.postedAt}
            </span>
          ...
          >
            <Bookmark
              ...
              className={cn(
                ...
                bookmarked ? "fill-gray-900 stroke-gray-900" : "fill-none stroke-gray-400",
              )}
            />
          </button>
...
// ─── JobListingsTable (main export) ───────────────────────────────────────────
/**
 * Responsive job listings display component.
 *
 * @param {{ jobs?: JobListing[] }} props
 *
 * Usage:
 *   <JobListingsTable />                     // uses built-in sample data
 *   <JobListingsTable jobs={myJobsArray} />  // pass your own data
 */
export default function JobListingsTable({ jobs = DEFAULT_JOBS }) {
  // Pre-select the last card (matches the design screenshot)
  const [selectedId, setSelectedId] = useState(jobs.at(-1)?.id ?? null);

  return (
    <div className="min-h-screen bg-slate-100 px-3 sm:px-4 py-6 sm:py-8 flex justify-center">
      <div className="w-full max-w-[860px] flex flex-col gap-2.5">
        {jobs.map((job) => (
          <JobCard
            key={job.id}
            job={job}
